...
const Index = () => {
  ...
  return <div className="min-h-screen bg-background">
      {!showSidebar && <Navigation isAuthenticated={isAuthenticated} userName={isAuthenticated ? "Gautham US" : undefined} />}
      
      {showSidebar ? <div className="flex h-screen">
          <Sidebar activeView={currentView} onNavigate={view => setCurrentView(view as ViewType)} userName="Gautham US" />
      ...
      <div className="fixed bottom-4 right-4 z-50 flex flex-col space-y-2">
        <button onClick={() => setCurrentView('landing')} className="px-3 py-1 text-xs bg-primary text-primary-foreground rounded-md hover:bg-primary-hover">
          Landing
        </button>
        <button onClick={() => setCurrentView('register')} className="px-3 py-1 text-xs bg-success text-success-foreground rounded-md hover:bg-accent-hover">
          Register
        </button>
        <button onClick={() => {
